...
impl<'a> InstallMethod<'a> {
    ...
    fn run(&self, path: &Path, notify_handler: &dyn Fn(Notification<'_>)) -> Result<bool> {
        ...
        match self {
            ...
            InstallMethod::Dist(opts) => {
                ...
                let maybe_new_hash = dist::update_from_dist(prefix, opts)?;
    ...
    fn dest_path(&self) -> PathBuf {
        match self {
            ...
            InstallMethod::Dist(DistOptions { cfg, toolchain, .. }) => {
                cfg.toolchain_path(&(*toolchain).into())
            }
        }
    }
}
